...
export class WebcamComponent implements OnInit {
  ...
  uploadImage(): void {
    ...
    this.http.post(`http://localhost:8089/api/facial/upload?id=${this.user.idUser}`, formData).subscribe({
      next: (res) => {
  ...
  sendImage(): void {
    ...
    this.http.post<{ result: string }>('http://localhost:8089/api/facial/compare-faces', formData).subscribe({
      next: (res) => {
